...
// yalnız bu sözlər varsa şəkil/cədvəl bağla
const FIGURE_HINT =
...
function parseQuestionBlock(blockLines: Array<{ text: string; yPx: number }>) {
  ...
  if (optStarts.length >= 2) {
    ...
    for (let i = 0; i < optStarts.length; i++) {
      ...
      const yStart = s.y
      const yEnd = i + 1 < optStarts.length ? optStarts[i + 1].y : (chunk[chunk.length - 1]?.yPx ?? yStart) + 1
  ...
  if (m2.length >= 2) {
    ...
    for (let i = 0; i < uniq.length; i++) {
      ...
      const yEnd = next ? (letterToY.get(next.letter) ?? yStart + 1) : yStart + 1
...
export async function parseQuestionsFromPages(pages: PdfPageData[]): Promise<DraftQuestion[]> {
  ...
  for (const pg of pages) {
    ...
    for (let si = 0; si < starts.length; si++) {
      ...
      const qFigures = figs
        .filter((f) => overlapLen(f.yTop, f.yBottom, qStartY, qEndY, 18) >= 18)
      ...
      // ---- şəkil bağlama (çoxlu + performanslı) ----
      const stemWants = wantsFigure(parsed.stem)
      const optWants = parsed.options.map((o) => wantsFigure(o.text))

      const usedFigureKeys = new Set<string>()
      function figKey(f: any) {
        return `${Math.round(f.yTop)}::${Math.round(f.area)}`
      }
      ...
      function pickTopFiguresForBucket(
        ...
        const candidates = qFigures
          .map((f) => ({ f, ov: overlapLen(f.yTop, f.yBottom, bucket.y1, bucket.y2, 12) }))
      ...
      if (qFigures.length) {
        ...
        const stemMinArea = Math.floor((pg.heightPx ?? 2000) * 20)
        const stemPicked = pickTopFiguresForBucket(stemBucket, stemWants ? 3 : 0, 24, stemMinArea)
        ...
        if (parsed.options.length >= 2) {
          for (let oi = 0; oi < parsed.options.length; oi++) {
            ...
            const optMinArea = Math.floor((pg.heightPx ?? 2000) * 14)
            const picked = pickTopFiguresForBucket({ y1: b.y1, y2: b.y2 }, 2, 26, optMinArea)
            if (picked.length) optToUrls[oi] = picked
          }
        }

        // ✅ fallback: mətn hint verməsə də, iri figure varsa bağla
        if (!stemUrls.length) {
          const qBucket = { y1: qStartY, y2: qEndY }
          const fallbackMinArea = Math.floor((pg.heightPx ?? 2000) * 26)
          const fallbackPicked = pickTopFiguresForBucket(qBucket, 2, 30, fallbackMinArea)
      ...
      const optionDrafts: DraftOption[] =
        parsed.options.length >= 2
          ? parsed.options.slice(0, 5).map((o, oi) => {
